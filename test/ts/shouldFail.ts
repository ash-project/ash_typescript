...
const invalidFieldNames = await getTodo({
  ...
  calculations: {
    self: {
      ...
      fields: ["id", "title", "nonExistentField", "anotherBadField"],
      calculations: {
        self: {
...
const arrayInsteadOfObject = await getTodo({
  fields: ["id"],
  // @ts-expect-error - calculations should be an object, not an array
  calculations: [
    {
      self: {
        calcArgs: { prefix: "test_" },
        ...
        self: {
          ...
          calculations: {
            self: {
              calcArgs: { prefix: "level3_" },
              fields: ["id"],
              // @ts-expect-error - calculations property should not exist at this level for this test case
              calculations: {
                invalidDeepNesting: {
