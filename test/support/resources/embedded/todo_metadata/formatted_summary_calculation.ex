defmodule AshTypescript.Test.TodoMetadata.FormattedSummaryCalculation do
  ...
  def load(_query, _opts, _context) do
    [:category, :subcategory, :priority_score, :status, :is_urgent, :deadline, :tags, :custom_fields]
  end
  ...
  defp build_short_summary(record, include_metadata) do
    ...
    base = if record.subcategory do
      "#{base} > #{record.subcategory}"
    else
      base
    end
    
    base = if record.is_urgent do
      "🚨 #{base}"
    else
      base
    end
  ...
  defp build_detailed_summary(record, include_metadata) do
    ...
    category_part = if record.subcategory do
    ...
    status_part = if record.is_urgent do
      "#{status_part} (URGENT)"
    else
      status_part
    end
    ...
    # Priority
    parts = if record.priority_score do
    ...
    # Deadline
    parts = if record.deadline do
      days_until = Date.diff(record.deadline, Date.utc_today())
      deadline_text = case days_until do
    ...
    # Tags
    parts = if length(record.tags || []) > 0 do
    ...
    # Metadata
    parts = if include_metadata && record.custom_fields && map_size(record.custom_fields) > 0 do
  ...
  defp build_json_summary(record, include_metadata) do
    ...
    data_with_deadline = if record.deadline do
      Map.put(base_data, :deadline, Date.to_string(record.deadline))
    else
      base_data
    end
    
    data_with_tags = if length(record.tags || []) > 0 do
      Map.put(data_with_deadline, :tags, record.tags)
    else
      data_with_deadline
    end
    
    final_data = if include_metadata && record.custom_fields && map_size(record.custom_fields) > 0 do
