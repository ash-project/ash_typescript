defmodule AshTypescript.Test.TodoMetadata.AdjustedPriorityCalculation do
  ...
  def calculate(records, opts, _context) do
    ...
    Enum.map(records, fn record ->
      ...
      adjusted = if record.is_urgent do
      ...
      adjusted = if deadline_factor && record.deadline do
        days_until_deadline = Date.diff(record.deadline, Date.utc_today())
        case days_until_deadline do
          days when days <= 1 -> adjusted + 20  # Very urgent
          days when days <= 7 -> adjusted + 10  # Urgent
          days when days <= 30 -> adjusted + 5  # Somewhat urgent
          _ -> adjusted
        end
      else
        adjusted
