...
defmodule AshTypescript.ControllerResource.RouterIntrospectionTest do
  use ExUnit.Case

  alias AshTypescript.ControllerResource.Codegen.{RouteConfigCollector, RouterIntrospector}

  @moduletag :ash_typescript

  defp routes_config do
    RouteConfigCollector.get_controller_resources(:ash_typescript)
  end
  ...
  describe "single mount router introspection" do
    test "introspects test router and matches routes to DSL config" do
      ...
      assert length(route_infos) == 5

      auth = Enum.find(route_infos, &(&1.route_action.name == :auth))
      assert auth.path == "/auth"
      assert auth.method == :get
      assert auth.path_params == []
      assert auth.scope_prefix == nil

      provider_page = Enum.find(route_infos, &(&1.route_action.name == :provider_page))
      ...
      login = Enum.find(route_infos, &(&1.route_action.name == :login))
      assert login.path == "/auth/login"
      assert login.method == :post
      assert login.scope_prefix == nil

      update_provider = Enum.find(route_infos, &(&1.route_action.name == :update_provider))
  ...
  describe "multi-mount router introspection" do
    test "returns multiple entries per route with scope prefixes" do
      ...
      assert length(route_infos) == 10

      auth_routes = Enum.filter(route_infos, &(&1.route_action.name == :auth))
      ...
      provider_routes = Enum.filter(route_infos, &(&1.route_action.name == :provider_page))
