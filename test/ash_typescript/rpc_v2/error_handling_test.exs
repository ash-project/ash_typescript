defmodule AshTypescript.RpcV2.ErrorHandlingTest do
  use ExUnit.Case

  alias AshTypescript.RpcV2.{ErrorBuilder, Pipeline}
  ...
  describe "field validation error messages" do
    ...
    test "simple calculation with spec error provides guidance" do
      error = {:invalid_fields, {:simple_calculation_with_spec, :is_overdue, %{"invalid" => "spec"}}}
    ...
    test "unsupported field combination error shows all context" do
      error = {:invalid_fields, {:unsupported_field_combination, :relationship, :user, "invalid_spec"}}
  ...
  describe "ash framework error handling" do
    test "ash exception error preserves framework details" do
      ...
      assert String.contains?(response.message, "Invalid")  # Uses Exception.message/1
      ...
      for error <- errors_with_suggestions do
        ...
        case response.details do
          %{suggestion: suggestion} ->
            ...
            has_action_word = Enum.any?(action_words, fn word ->
  ...
  describe "end-to-end error handling in pipeline" do
    test "pipeline returns properly structured error responses" do
      ...
      assert {:error, error_response} = Pipeline.parse_request_strict(:ash_typescript, conn, params)
