  ...
  describe "RPC Integration - End-to-End Field Processing" do
    ...
    test "create_user action respects fields specification", %{conn: conn} do
      ...
      result = AshTypescript.Rpc.run_action(:ash_typescript, conn, %{
        ...
        "fields" => ["id", "name"]  # Only request specific fields
      })
      ...
      refute Map.has_key?(data, "email")  # Not requested
      refute Map.has_key?(data, "active")  # Not requested
      refute Map.has_key?(data, "isSuper admin")  # Not requested
    ...
    test "create_todo action with nested relationship fields", %{conn: conn} do
      ...
      result = AshTypescript.Rpc.run_action(:ash_typescript, conn, %{
        ...
        "fields" => [
          "id",
          "title",
          %{"user" => ["name", "email"]},  # Nested relationship fields
          %{"metadata" => ["category", "priorityScore"]}  # Embedded resource fields (client format)
        ]
      })
      ...
      assert %{
        ...
        "metadata" => %{
          "category" => "testing",
          "priorityScore" => 7  # Should be camelCase formatted
        }
      } = data

      # Verify field filtering at all levels
      refute Map.has_key?(data, "completed")  # Root level - not requested
      refute Map.has_key?(data, "description")  # Root level - not requested
      refute Map.has_key?(data["user"], "id")  # Nested level - not requested
      refute Map.has_key?(data["user"], "active")  # Nested level - not requested
    ...
    test "list_todos action with complex field specifications", %{conn: conn} do
      ...
      assert length(todos) >= 3  # At least our test todos

      # Find our test todos
      test_todos = Enum.filter(todos, fn todo -> 
    ...
    test "update_todo action preserves field specification behavior", %{conn: conn} do
      ...
      create_result = AshTypescript.Rpc.run_action(:ash_typescript, conn, %{
        "action" => "create_todo",
        "input" => %{
      ...
      result = AshTypescript.Rpc.run_action(:ash_typescript, conn, %{
        ...
        "fields" => [
          "id",
          "title",
          %{"metadata" => ["category"]}  # Only request category from metadata
        ]
      })
      ...
      refute Map.has_key?(data, "completed")  # Not requested
      refute Map.has_key?(data, "user")  # Not requested
      refute Map.has_key?(data["metadata"], "priorityScore")  # Not requested from metadata
    ...
    test "field specification with calculations and formatting", %{conn: conn} do
      ...
      result = AshTypescript.Rpc.run_action(:ash_typescript, conn, %{
        ...
        "fields" => [
          "id",
          "title",
          %{"user" => ["name"]}  # Use available field instead of non-existent calculation
        ]
      })
    ...
    test "error handling preserves field specification contract", %{conn: conn} do
      ...
      result = AshTypescript.Rpc.run_action(:ash_typescript, conn, %{
        ...
        "input" => %{
          ...
          "userId" => "invalid-uuid"  # Invalid: bad UUID
        },
    ...
    test "empty fields specification returns minimal data", %{conn: conn} do
      ...
      result = AshTypescript.Rpc.run_action(:ash_typescript, conn, %{
        ...
        "fields" => []  # Empty fields specification
      })
