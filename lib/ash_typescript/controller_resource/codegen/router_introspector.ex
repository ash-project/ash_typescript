...
defmodule AshTypescript.ControllerResource.Codegen.RouterIntrospector do
  @moduledoc """
  ...
  controller actions generated by the ControllerResource extension.
    ...
    Enum.flat_map(routes_config, fn {resource, controller_module, routes} ->
      Enum.flat_map(routes, fn route ->
        ...
        case matches do
          ...
          [single] ->
            [build_route_info(resource, controller_module, route, single, nil)]

          multiple ->
            build_multi_mount_route_infos(resource, controller_module, route, multiple)
        end
      end)
    end)
  end

  defp build_route_info(resource, controller_module, route, router_route, scope_prefix) do
  ...
  defp build_multi_mount_route_infos(resource, controller_module, route, matches) do
    ...
    Enum.map(matches, fn match ->
      ...
      build_route_info(resource, controller_module, route, match, prefix)
    end)
  end
