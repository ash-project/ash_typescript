defmodule AshTypescript.RpcV2.Pipeline do
  @moduledoc """
  ...
  alias AshTypescript.RpcV2.{Request, FieldParserV2, ResultFilterV2}
  ...
  def parse_request_strict(otp_app, conn, params) do
    with {:ok, {resource, action}} <- discover_rpc_action(otp_app, params),
         {:ok, tenant} <- resolve_tenant(resource, conn, params),
         {:ok, {select, load, template}} <- parse_fields_strict(params, resource),
         {:ok, input} <- parse_input_strict(params, action),
         {:ok, pagination} <- parse_pagination_strict(params) do
  ...
  def filter_result_fields(ash_result, %Request{} = request) do
    case ash_result do
      :ok ->
        {:ok, %{}}
      {:ok, result} ->
        filtered = ResultFilterV2.extract_fields(result, request.extraction_template)
        {:ok, filtered}
  ...
  """
  @spec format_output(term(), Request.t()) :: {:ok, term()} | {:error, term()}
  def format_output(filtered_result, %Request{} = _request) do
    # Apply output formatting using the configured formatter
    formatter = Rpc.output_field_formatter()
    formatted_result = format_field_names(filtered_result, formatter)
    {:ok, formatted_result}
  end
  ...
  defp find_rpc_action(otp_app, action_name) when is_binary(action_name) do
    otp_app
    |> Ash.Info.domains()
  ...
  defp parse_fields_strict(params, resource) do
    ...
    case FieldParserV2.parse_requested_fields_strict(client_fields, resource, formatter) do
  ...
  defp parse_input_strict(params, action) do
    ...
    raw_input_with_pk = 
      if params["primary_key"] && action.get? do
