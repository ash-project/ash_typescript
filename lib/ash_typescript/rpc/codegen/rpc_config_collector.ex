...
defmodule AshTypescript.Rpc.Codegen.RpcConfigCollector do
  ...
  def get_rpc_resources_and_actions(otp_app, resource_lookup) do
    ...
    get_rpc_resources_and_actions(otp_app, resource_lookup, action_lookup)
  end

  def get_rpc_resources_and_actions(otp_app, _resource_lookup, action_lookup) do
    otp_app
    |> Ash.Info.domains()
    |> Enum.flat_map(fn domain ->
      rpc_config = AshTypescript.Rpc.Info.typescript_rpc(domain)
      ...
      Enum.flat_map(rpc_config, fn %{resource: resource, rpc_actions: rpc_actions} ->
        Enum.map(rpc_actions, fn rpc_action ->
          action = Map.fetch!(action_lookup, {resource, rpc_action.action})
  ...
  def get_rpc_resources_and_actions_with_context(otp_app, resource_lookup) do
    action_lookup = AshTypescript.action_lookup(otp_app)
    get_rpc_resources_and_actions_with_context(otp_app, resource_lookup, action_lookup)
  end

  def get_rpc_resources_and_actions_with_context(otp_app, _resource_lookup, action_lookup) do
    otp_app
    |> Ash.Info.domains()
    |> Enum.flat_map(fn domain ->
      rpc_config = AshTypescript.Rpc.Info.typescript_rpc(domain)
    ...
    |> Enum.flat_map(fn domain ->
      ...
      Enum.flat_map(rpc_config, fn %{resource: resource, typed_queries: typed_queries} ->
        Enum.map(typed_queries, fn typed_query ->
          action = Map.fetch!(action_lookup, {resource, typed_query.action})
