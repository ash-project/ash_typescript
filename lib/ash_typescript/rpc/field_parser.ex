defmodule AshTypescript.Rpc.FieldParser do
  ...
  """
  def classify_and_process(field_atom, field_spec, %Context{} = context) when is_atom(field_atom) do
    case classify_field(field_atom, context.resource) do
      ...
      :complex_calculation when is_map(field_spec) ->
        ...
        {load_entry, field_specs} = LoadBuilder.build_calculation_load_entry(field_atom, field_spec, context)
        {:calculation_load, load_entry, field_specs}
      ...
      :relationship when is_list(field_spec) ->
        ...
        nested_load = process_relationship_fields(field_atom, target_resource, field_spec, context.formatter)
        {:load, nested_load}
      ...
      :embedded_resource when is_list(field_spec) ->
        ...
        embedded_load_items = process_embedded_fields(embedded_module, field_spec, embedded_context)
  ...
  defp process_embedded_field(field_name, embedded_module, %Context{} = context) when is_binary(field_name) do
    field_atom = FieldFormatter.parse_input_field(field_name, context.formatter)
    ...
    case classify_field(field_atom, embedded_module) do
      ...
      :simple_attribute -> nil  # Automatically included, skip
      :unknown -> nil  # Unknown field - skip for safety
    end
  end
  
  defp process_embedded_field(field_map, embedded_module, %Context{} = context) when is_map(field_map) do
    case Map.to_list(field_map) do
      [{field_name, nested_fields}] when is_binary(field_name) and is_list(nested_fields) ->
        ...
        case classify_field(field_atom, embedded_module) do
          :relationship ->
            ...
            process_relationship_fields(field_atom, target_resource, nested_fields, context.formatter)
          ...
          :embedded_resource ->
            ...
            nested_load_items = process_embedded_fields(nested_embedded_module, nested_fields, nested_context)
            
            case nested_load_items do
              [] -> nil  # No loadable items in nested embedded resource
              items -> {field_atom, items}
            end
            
          _ -> nil  # Not a loadable complex field
        end
      ...
      [{field_name, calc_spec}] when is_binary(field_name) and is_map(calc_spec) ->
        ...
        case classify_field(field_atom, embedded_module) do
          :complex_calculation ->
            # Complex calculation with arguments
            embedded_context = Context.child(context, embedded_module)
            {load_entry, _field_specs} = LoadBuilder.build_calculation_load_entry(field_atom, calc_spec, embedded_context)
            load_entry
            
          _ -> nil  # Not a complex calculation
        end
        
      _ -> nil  # Invalid field map format
    end
  end
  
  defp process_embedded_field(_field, _embedded_module, _context) do
    nil  # Unknown field format
  end
