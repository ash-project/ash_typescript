...
defmodule AshTypescript.Rpc.Codegen do
  ...
  alias AshTypescript.Codegen.{FilterTypes, ResourceSchemas, SchemaCore, TypeAliases, TypeDiscovery, ZodSchemaGenerator}
  ...
  def generate_rpc_content(resources_and_actions, opts, codegen_opts) do
    ...
    #{TypescriptStatic.generate_imports(skip_zod: true, skip_valibot: true, output_file: Keyword.get(codegen_opts, :output_file))}
    #{shared_imports}
  ...
  """
  def generate_rpc_schemas(formatter, resources_and_actions) do
    resources_and_actions
    |> Enum.map(fn {resource, action, rpc_action} ->
      rpc_action_name = to_string(rpc_action.name)
      action = augment_action_with_rpc_settings(action, rpc_action, resource)
      SchemaCore.generate_action_schema(formatter, resource, action, rpc_action_name)
    end)
    |> Enum.reject(&(&1 == ""))
  end
