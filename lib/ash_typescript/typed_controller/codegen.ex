...
defmodule AshTypescript.TypedController.Codegen do
  ...
  def generate_controller_content(opts) do
    ...
    if routes_config == [] do
      ...
      generate_typescript_with_imports(route_infos, import_paths, shared_type_names, base_path)
    end
  end
  ...
  defp generate_typescript_with_imports(route_infos, import_paths, shared_type_names, base_path) do
    header = """
    ...
    static_code =
      if AshTypescript.typed_controller_mode() == :full do
        TypescriptStatic.generate_static_code(skip_zod: true, base_path: base_path)
      else
        if has_base_path do
